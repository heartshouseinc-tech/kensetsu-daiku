...
exports.askClaude = onRequest(
  ...
  async (req, res) => {
    ...
    // アクセスキー認証
    const reqKey = req.headers["x-access-key"];
    if (reqKey !== ACCESS_KEY) {
      return res.status(403).json({ error: "Forbidden" });
    }

    try {
      const { prompt, model, mediaContents, messages, useSearch } = req.body;
      const genAI = new GoogleGenerativeAI(GEMINI_API_KEY.value());
      const modelName = model || "gemini-2.5-flash";

      const tools = useSearch ? [{ googleSearch: {} }] : [];
      const geminiModel = genAI.getGenerativeModel({ model: modelName, tools });
